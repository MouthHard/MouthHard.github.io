<template>
	<!-- PC端 -->
	<div id="pcMusic" v-show="pcOrMobile == true">
		<div id="pcLeft">
			<h4 @click="changePorM">
	...
	<!-- 移动端 -->
	<div id="musicCon" @click.self="closeSongList(), closeShareSetting()" v-show="pcOrMobile == false">
...
songArr.forEach((e) => {
	if (e.lyric == "纯音乐请聆听") {
		lyricMap.set(i++, e.lyric);
	} else {
		lyricMap.set(i++, e.lyric.split("[").slice(1, e.lyric.split("[").length));
	}
});
...
const COList = (str) => {
	let arr = document.querySelectorAll("#" + str + " a");
...
const showsingleSongSetting = (index) => {
	document.querySelectorAll(".singleSongSetting").forEach((e) => {
...
const changePorM = () => {
	pcOrMobile.value = !pcOrMobile.value;
	if (pcOrMobile.value == false) {
		curPlayState = true;
	} else {
		curPlayState = false;
	}
...
onMounted(() => {
	...
	musicFoot.value.addEventListener("timeupdate", (e) => {
		...
		for (let i = 0; i < curLyric.length; i++) {
			let sumTime = e.target.currentTime.toFixed(3);
			let curSongTime = 0;
			while (sumTime > 60) {
				curSongTime += 1;
				sumTime -= 60;
			}
			// 时间整理成 xx:xx.xxx的格式方便进行对比
			curSongTime = curSongTime < 10 ? "0" + curSongTime : "" + curSongTime;
			sumTime = sumTime < 10 ? "0" + sumTime : "" + sumTime;
			curSongTime = curSongTime + ":" + sumTime;
			//如果当前audio时间 >歌词对应的时间，那么显示对应的歌词
			if (curSongTime > curLyric[i].itTime) {
...
#pcMusic {
	...
	#pcLeft {
		...
		a {
			...
			transition: 1s;
			&.router-link-active {
				box-shadow: inset -2rem 0 10px -4px #fff;
				text-align: center;
			}
			&:hover {
				cursor: pointer;
				background: linear-gradient(45deg, #1f75b3, #8a1515);
			}
		}
		#leftList {
			width: 100%;
	...
	#pcCenter {
		...
		box-shadow: 5px 0 15px -2px #000000;
		...
		div:nth-of-type(1) {
			...
			z-index: 15;
			top: 0;
			box-shadow: -15px 0 15px 3px #000000;
			...
			input {
				...
				outline-style: none;
				&:focus {
					box-shadow: inset 0 0 10px 2px #22adfe;
				}
			}
			svg,
			p,
...
.music-head {
	...
	button {
		...
		&:hover {
			...
			cursor: pointer;
			text-shadow: 0 0 5px white, 0 0 10px #0000f8, 0 0 15px #ffffff, 0 0 20px white;
...
.music-center {
	background: url("../assets/music-bg.jpg");
	background-size: 100%;
	height: 72.3%;
	position: relative;
	div:nth-child(1) {
	...
	img {
		...
		height: 100%;
		border-radius: 50%;
		animation: rotate360deg 20s infinite linear;
...
.music-foot {
	...
	justify-content: space-around;
	height: 22.5%;
	background: linear-gradient(to bottom, rgb(184, 202, 215), #fff, rgb(184, 202, 215));
	div:nth-child(1) {
		display: flex;
		justify-content: space-around;
	}
...
#songLiric {
	...
	.activeLyric {
		font-size: 1.3rem;
		color: #ffffff;
		// border-bottom: #1355b3 1px ridge;
		// box-shadow:inset 0 -5px 15px -15px rgb(1, 179, 255) ;
		// -webkit-box-reflect: below -10px -webkit-linear-gradient(transparent, transparent 5%, rgba(255, 255, 255, 0.2));
		text-shadow: 0 5px 5px rgb(1, 179, 255);
		transition: 1s;
	}
	scrollbar-width: none; /* Firefox */
	-ms-overflow-style: none; /* IE 10+ */
...
#songList {
	...
	height: 100%;
	background: #e5e5e5;
	box-shadow: -10px 0 10px 2px #818080;
	transition: 1s;
	color: #000;
	svg {
		transition: 1s;
	...
	.active {
		box-shadow: inset -1px 0 10px 1px #000000;
		img {
